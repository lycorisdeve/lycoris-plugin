...
async function loadHelpConfig() {
    try {
        ...
        if (helpCfgStat.isFile() || helpListStat.isFile()) {
            ...
            const fileUrl = `file://${helpCfgStat.isFile() ? HELP_CFG_PATH : HELP_LIST_PATH}?version=${version}`;
...
class HelpService {
    async help(e) {
        ...
        const custom = lodash.isArray(helpCfg?.helpCfg) ? { helpList: helpCfg.helpCfg, helpCfg: {} } : helpCfg || {};
        const mergedHelpCfg = lodash.defaults(diyCfg.helpCfg || {}, custom.helpCfg, sysCfg.helpCfg);
        const mergedHelpList = diyCfg.helpList || custom.helpList || sysCfg.helpList;

        // 过滤和转换帮助项目组数组
        const helpGroup = mergedHelpList.flatMap(group => {
        ...
        return { helpCfg: mergedHelpCfg, helpGroup, ...themeData, element: 'default' }
